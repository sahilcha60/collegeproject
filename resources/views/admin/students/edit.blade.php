...
<div class="max-w-lg mx-auto bg-white rounded-lg shadow p-6">
    <h3 class="text-lg font-semibold text-gray-700 mb-6">Edit Student</h3>
    ...
    <form action="{{ route('admin.students.update', $student) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="enrollment_no" value="{{ old('enrollment_no', $student->enrollment_no) }}"
        ...
        <div class="mb-6">
            ...
            <select name="semester_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                ...
                @foreach($semesters as $sem)
                    <option value="{{ $sem->id }}" {{ old('semester_id', $student->semester_id) == $sem->id ? 'selected' : '' }}>{{ $sem->name }}</option>
                @endforeach
            </select>
