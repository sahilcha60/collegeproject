...
@section('header_title', 'Edit Exam Schedule')
@section('content')
<div class="max-w-lg mx-auto bg-white rounded-lg shadow p-6">
    <h3 class="text-lg font-semibold text-gray-700 mb-6">Edit Exam Schedule</h3>
    ...
    <form action="{{ route('admin.exam-schedules.update', $examSchedule) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="exam_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                ...
                @foreach($exams as $exam)
                    <option value="{{ $exam->id }}" {{ old('exam_id', $examSchedule->exam_id) == $exam->id ? 'selected' : '' }}>{{ $exam->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="subject_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                ...
                @foreach($subjects as $subject)
                    <option value="{{ $subject->id }}" {{ old('subject_id', $examSchedule->subject_id) == $subject->id ? 'selected' : '' }}>{{ $subject->name }} ({{ $subject->semester->name ?? '' }})</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="exam_date" value="{{ old('exam_date', $examSchedule->exam_date) }}"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
            @error('exam_date')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">Start Time</label>
            <input type="time" name="start_time" value="{{ old('start_time', $examSchedule->start_time) }}"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
            @error('start_time')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>
        <div class="mb-6">
            <label class="block text-sm font-medium text-gray-700 mb-1">End Time</label>
            <input type="time" name="end_time" value="{{ old('end_time', $examSchedule->end_time) }}"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
