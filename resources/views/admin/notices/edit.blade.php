    ...
    <form action="{{ route('admin.notices.update', $notice) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="target" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 @error('target') border-red-500 @enderror">
                ...
                <option value="student" {{ old('target', $notice->target) == 'student' ? 'selected' : '' }}>Students</option>
                <option value="teacher" {{ old('target', $notice->target) == 'teacher' ? 'selected' : '' }}>Teachers</option>
