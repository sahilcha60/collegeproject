...
@section('header_title', 'Edit Exam')
@section('content')
<div class="max-w-lg mx-auto bg-white rounded-lg shadow p-6">
    <h3 class="text-lg font-semibold text-gray-700 mb-6">Edit Exam</h3>
    ...
    <form action="{{ route('admin.exams.update', $exam) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="name" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 @error('name') border-red-500 @enderror">
                ...
                @foreach(['Mid', 'Pre-board', 'Board'] as $opt)
                    <option value="{{ $opt }}" {{ old('name', $exam->name) == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 @error('type') border-red-500 @enderror">
                ...
                <option value="college"    {{ old('type', $exam->type) == 'college'    ? 'selected' : '' }}>College</option>
                <option value="university" {{ old('type', $exam->type) == 'university' ? 'selected' : '' }}>University</option>
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="start_date" value="{{ old('start_date', $exam->start_date) }}"
        ...
        <div class="mb-6">
            ...
            <input type="date" name="end_date" value="{{ old('end_date', $exam->end_date) }}"
