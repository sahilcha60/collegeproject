    ...
    <form action="{{ route('admin.exams.update', $exam) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="name" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 @error('name') border-red-500 @enderror">
                ...
                <option value="Mid" {{ old('name', $exam->name) == 'Mid' ? 'selected' : '' }}>Mid</option>
                <option value="Pre-board" {{ old('name', $exam->name) == 'Pre-board' ? 'selected' : '' }}>Pre-board</option>
                <option value="Board" {{ old('name', $exam->name) == 'Board' ? 'selected' : '' }}>Board</option>
            </select>
